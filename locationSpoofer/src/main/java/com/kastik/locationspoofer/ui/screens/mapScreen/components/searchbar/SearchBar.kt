...
fun TopSearchBar(
    ...
    searchResults: List<PlaceDomain>,
    savedPlaces: List<PlaceDomain>,
    onSearchQueryChanged: (String) -> Unit,
    onSearchResultClick: (PlaceDomain) -> Unit,
    onChipClicked: (LatLngDomain) -> Unit
) {
            ...
            SearchBar(
                ...
                LazyColumn {
                    items(searchResults) { result ->
                        ListItem(
                            headlineContent = { result.name?.let { Text(it) } },
                            leadingContent = {
                                Icon(
...
fun TopSearchBarPreview() {
    TopSearchBar(
        ...
        savedPlacesList =
            SavedPlaces.newBuilder().addPlace(
                SavedPlace.newBuilder().setPlace(
                    Place.newBuilder().setName("Central park").build()
                ).build()
            ).build())
}
 */